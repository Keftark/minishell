...
/*   Updated: 2023/09/26 15:21:26 by cpothin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
static int	expand_variable(t_data *data, char *new_input, char *input, int *i)
{
	char	*var_value;
	int		size;
	int		j;
	...
	*i += 1;
	...
	while (input[*i + size] && input[*i + size] != ' ' && input[*i
		+ size] != '\"' && !is_char(QUOTES, input[*i + size]) && input[*i
		+ size] != '$')
		size += 1;
